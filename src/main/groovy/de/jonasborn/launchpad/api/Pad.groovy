...
enum Pad implements Pressable {
    ...
    static Pad find(int x, int y) {
        return values().find { it.x == x && it.y == y }
    }

    static Pad find(ShortMessage message) {
        def v = values().find { it.command == message.command && it.getCode() == message.data1 }
        return v
    }
